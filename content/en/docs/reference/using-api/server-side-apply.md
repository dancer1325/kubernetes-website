...
* [ServerSideApply feature gate](../command-line-tools-reference/feature-gates/ServerSideApply.md)

* [Kubernetes object's fields](../glossary/object.md)
  * can be managed -- through -- MULTIPLE appliers 
    * _Example of appliers:_ users, [controllers](../glossary/controller.md), autoscalers, ..

* Server-Side Apply
  * provides
    * 👀mechanism / your cluster's control plane can track changes | object's fields👀
      * / EACH specific resource
      * -- through -- declarative configuration
  * [vs client side](#comparison-with-client-side-apply)

* clients
  * create & modify [Kubernetes objects](../glossary/object.md)
    * declaratively
    * by submitting their _fully specified intent_

* fully specified intent
  * == partial object / ONLY includes the fields & values / user has an opinion
  * intent either 
    * creates a NEW object 
      * default values / unspecified fields, OR
    * [combined + existing object](#merge-strategy) 
...
user relies on and expects the value of the field not to change
* The user who
last made an assertion about the value of a field will be recorded as the
current field manager
* This can be done by changing the field manager
...
that field
* Any subsequent attempt to change the value of the shared field, by any of
the appliers, results in a conflict
* Shared field owners may give up ownership
...
* _conflict_
  * == special status error | `Apply` operation tries to change a field / another manager ALSO claims to manage
...
* However, Kubernetes has a bug where it uses a YAML
parser that does not fully implement the YAML specification
* Some JSON escapes may
not be recognized.
{{< /note >}}
...
operations
* Finally, when using the `Apply` operation you cannot define `managedFields` in
...
more stable object lifecycle
* Server-Side Apply tries to merge fields based on
the actor who manages them instead of overruling based on values
* This way
...
request body if it is specified in both places
* If the set of items present in
the applied config is not a superset of the items applied by the same user last
time, each missing item not managed by any other appliers is removed
* For
...
defining _merge strategy markers_
* These markers describe the merge strategy supported
...
| --------------- | ---------------------------- | ------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `//+listType`   | `x-kubernetes-list-type`     | `atomic`/`set`/`map`                             | Applicable to lists
* `set` applies to lists that include only scalar elements
* These elements must be unique
* `map` applies to lists of nested types only
* The key values (see `listMapKey`) must be unique in the list
* `atomic` can apply to any list
* If configured as `atomic`, the entire list is replaced during merge
* At any point in time, a single manager owns the list
* If `set` or `map`, different managers can manage entries separately
* |
| `//+listMapKey` | `x-kubernetes-list-map-keys` | List of field names, e.g
* `["port", "protocol"]` | Only applicable when `+listType=map`
* A list of field names whose values uniquely identify entries in the list
* While there can be multiple keys, `listMapKey` is singular because keys need to be specified individually in the Go type
* The key fields must be scalars
*                                                                                                                                                                                |
| `//+mapType`    | `x-kubernetes-map-type`      | `atomic`/`granular`                              | Applicable to maps
* `atomic` means that the map can only be entirely replaced by a single manager
* `granular` means that the map supports separate managers updating individual fields
*                                                                                                                                                                                                                                                                  |
...
without incrementing its API version
* Changing the topology of types,
by upgrading the cluster or updating the CRD, has different consequences when
updating existing objects
* There are two categories of changes: when a field goes from
...
of these types
* This means that any further change to these objects
...
* Because of that, no conflict will be produced
when objects have these fields updated
...
simplify the update logic of your controller
* The main differences with a
...
* Let's look at how to safely transfer
...
- (basic) Leave `replicas` in the configuration; when the HPA eventually writes to that
  field, the system gives the user a conflict over it
* At that point, it is safe
...
The user applies that manifest using a private field manager name
* In this example,
...
If the apply results in a conflict with the HPA controller, then do nothing
* The
...
* Once the managers share ownership of the field, one of them
...
rather than a user's last applied state
* This means that as a side effect of
...
state
* If they don't, they get a conflict the next time they apply
* Any of the
...
overwritten by other users are left in an applier's local config
* These values
...
non-default field manager, as seen in the following example
* The default field
...
unofficial `application/apply-patch+yaml` content type
* Users of Server-Side
Apply can send partially specified objects as YAML as the body of a `PATCH` request
to the URI of a resource
*  When applying a configuration, you should always include all the
fields that are important to the outcome (such as a desired state) that you want to define.

All JSON messages are valid YAML
* Therefore, in addition to using YAML request bodies for Server-Side Apply requests, you can also use JSON request bodies, as they are also valid YAML.
...
other than **apply**
* This can be done by overwriting the `managedFields` field
with an empty entry
* Two examples are:
...
object
* Note that setting the `managedFields` to an empty list will not
reset the field
* This is on purpose, so `managedFields` never get stripped by
